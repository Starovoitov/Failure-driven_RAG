...
class TestIngestionCleaner(unittest.TestCase):
    def test_cleanup_index_only(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            result = cleanup_faiss_db(persist_directory=str(root), index_name="idx", drop_persist_directory=False)
    ...
    def test_cleanup_full_directory(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            result = cleanup_faiss_db(persist_directory=str(root), index_name="idx", drop_persist_directory=True)
    ...
    def test_cleanup_drop_directory_without_index(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            result = cleanup_faiss_db(persist_directory=str(root), index_name="not-there", drop_persist_directory=True)
            self.assertFalse(result["index_deleted"])
            self.assertTrue(result["directory_deleted"])
